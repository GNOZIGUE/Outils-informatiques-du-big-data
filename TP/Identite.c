...
int main(int argc, char **argv) {
    ...
    MPI_Aint base_address = indices[0];
    ...
    do {
        if (rank == 0) {
            ...
            fflush(stdout);
            if (scanf("%14s %14s %d", value.nom, value.prenom, &value.age) != 3) {
        ...
        printf("Processus %d a recu Nom: %s, Prénom: %s et Age: %d\n", rank, value.nom, value.prenom, value.age);
        fflush(stdout);
